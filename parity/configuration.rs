...
mod tests {
	...
	fn should_parse_rpc_settings_with_geth_compatiblity() {
		...
		// when
		let conf1 = parse(&["openethereum", "-j",
						 "--jsonrpc-port", "8000",
						 "--jsonrpc-interface", "all",
						 "--jsonrpc-cors", "*",
						 "--jsonrpc-apis", "web3,eth"
						 ]);
		let conf2 = parse(&["openethereum", "--rpc",
							"--rpcport", "8000",
							"--rpcaddr", "all",
							"--rpccorsdomain", "*",
							"--rpcapi", "web3,eth"
	...
	fn should_parse_ui_configuration() {
		...
		let conf1 = parse(&["openethereum", "--ui-path=signer", "--ui-no-validation"]);
		let conf2 = parse(&["openethereum", "--ui-path=signer", "--ui-port", "3123"]);
		let conf3 = parse(&["openethereum", "--ui-path=signer", "--ui-interface", "test"]);
		let conf4 = parse(&["openethereum", "--ui-path=signer", "--force-ui"]);
